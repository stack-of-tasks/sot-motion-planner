...
#define DELAY_2 0.2 //*1.5/2.0

using namespace std;

CnewPGstepStudy::CnewPGstepStudy(double stepLength)
{
  stepLength_ = stepLength;
  delay1 = 0.005;
  delay2 = 0.2 * stepLength_ /2.0;
}
...
void CnewPGstepStudy::drawSeqStepFeatures(ofstream & fb, double incrTime, double zc, double g, double t1, double t2, double t3, double t4, double t5, vector<double> vect_input, char leftOrRightFootStable, double coefFeet)
{
  ...
  double downBound=0.0, upBound=0.0, leftBound=0.0, rightBound=0.0;
  ...
  double centre_x=0.0;
  double centre_y=0.0;
  double abs_orientation=0.0;	

  for(int i=0; i < (int) (vect_input.size()-6)/4+2 ; i++)
    {

      double abs_orientationRAD=0.0;
  ...
  for(unsigned int i=0; i < stepF.size-1 ; i++)
    {
					       ...
					       ) 
{
  ...
  unsigned int delayInt = (unsigned int) (0.0001 + abs(negativeSlideTime)/stepF2.incrTime);
...
void CnewPGstepStudy::produceOneStepFeatures(StepFeatures & stepF, double incrTime, double zc, double g, double t1, double t2, double t3, double t4, double t5, vector<double> vectStep_input, char leftOrRightFootStable)
{
  ...
  genFOOTposition(footXtraj, footYtraj, incrTime, vectStep_input[3], vectStep_input[4], vectStep_input[0]+vectStep_input[7], vectStep_input[1]+vectStep_input[8], delay2, t1, t2, t3, t4, t5, '2');

  vector<double> footHeight;
  genFOOTheight(footHeight, incrTime, stepHeight, delay1, t1, t2, t3, t4, t5);

  vector<double> footOrient;
  genFOOTorientation(footOrient, incrTime, vectStep_input[5], vectStep_input[9], delay2, t1, t2, t3, t4, t5, '2');
  ...
  genWAISTorientation(waistOrient, incrTime, 0, vectStep_input[9], delay1, t1, t2, t3, t4, t5, '2');
...
void CnewPGstepStudy::produceOneUPHalfStepFeatures(StepFeatures & stepF, double incrTime, double zc, double g, double t1, double t2, double t3, vector<double> vectUPHalfStep_input, char leftOrRightFootStable)
{
  ...
  genFOOTposition(footXtraj, footYtraj, incrTime, vectUPHalfStep_input[3], vectUPHalfStep_input[4], vectUPHalfStep_input[0], vectUPHalfStep_input[1]+leftRightCoef*vectUPHalfStep_input[6], delay2, t1, t2, t3, t3, t3, 'u');

  vector<double> footHeight;
  genFOOTdownUPheight(footHeight, incrTime, vectUPHalfStep_input[7], delay1, t1, t2, t3);

  vector<double> footOrient;
  genFOOTorientation(footOrient, incrTime, vectUPHalfStep_input[5], 0, delay2, t1, t2, t3, t3, t3, 'u');
  ...
  vector<double> waistOrient;
  genWAISTorientation(waistOrient, incrTime, 0, 0, delay1, t1, t2, t3, t3, t3, 'u');
...
void CnewPGstepStudy::produceOneDOWNHalfStepFeatures(StepFeatures & stepF, double incrTime, double zc, double g, double t1, double t2, double t3, vector<double> vectDOWNHalfStep_input, char leftOrRightFootStable)
{
  ...
  int leftRightCoef = 0;	
  if(leftOrRightFootStable == 'L') leftRightCoef = -1; else leftRightCoef = 1;
  genFOOTposition(footXtraj, footYtraj, incrTime, 0, leftRightCoef*vectDOWNHalfStep_input[0], vectDOWNHalfStep_input[2], vectDOWNHalfStep_input[3], delay2, 0, 0, t1, t2, t3, 'd');

  vector<double> footHeight;
  genFOOTupDOWNheight(footHeight, incrTime, vectDOWNHalfStep_input[1], delay1, t1, t2, t3);

  vector<double> footOrient;
  genFOOTorientation(footOrient, incrTime, 0, vectDOWNHalfStep_input[4], delay2, 0, 0, t1, t2, t3, 'd');

  vector<double> waistOrient;
  genWAISTorientation(waistOrient, incrTime, 0, vectDOWNHalfStep_input[4], delay1, 0, 0, t1, t2, t3, 'd');
...
{	
  ...
  tmpEleven[9] = (vectSteps_input[0]*2)*sin(-vectSteps_input[5]*PI/180)+(vectSteps_input[1]*2)*cos(-vectSteps_input[5]*PI/180) ;
  tmpEleven[10] = -vectSteps_input[5];
  //printf("NNPS : %f %f %f\n", tmpEleven[8], tmpEleven[9], tmpEleven[10]);
  ...
  for(unsigned int i = 1; i <= (vectSteps_input.size()-6)/7; i++) {		
    ...
    //printf("%f %f | %f %f | %f %f %f\n", tmpEleven[0], tmpEleven[1] , tmpEleven[6], tmpEleven[7] , tmpEleven[8], tmpEleven[9] , tmpEleven[10]);
