...
static const double T = 0.8; // ------ Moi -------- Walking period 
...
protected:
  ...
  /// test file
   ofstream fichier("SwayMotionCorrectionTestOnRobot.txt", ios::trunc); 
   //
};
...
SwayMotionCorrection::updateVelocity (ml::Vector& velWaist, int t)
{
  ...
  // ---------- Moi ----------  sign of bk changed
  vpColVector bk (6);
  bk[0] = dcom[0] - inputComVel[0];
  bk[1] = dcom[1] - inputComVel[1];
  bk[2] = bk[3] = bk[4] = 0.;
  bk[5] = dcom[2] - inputComVel[2];
  ...
  // ------------ Moi ----------------
  vpColvector E_tT; // == int(E_ , t-T, T)/T
  ...
  //------ Moi -------- add sway motion correction to the waist velocity
  fichier << "Vitesse Visp :  x =  " << velWaistVisp[0] << "    y =  " << velWaistVisp[1] << endl;
  fichier << "Vitesse integralLbk :  x =  " << lamda_*task_.L.pseudoInverse()*intergralLbk[0] << "    y =  " << lamda_*task_.L.pseudoInverse()*intergralLbk[1] << endl;
  fichier << "Vitesse E_tT :  x =  " << -lamda_*task_.L.pseudoInverse()*E_tT[0] << "    y =  " << -lamda_*task_.L.pseudoInverse()*E_tT[1] << endl;
  velWaistVisp = velWaistVisp + lamda_*task_.L.pseudoInverse()*(intergralLbk_ - E_tT);
  fichier << "Vitesse calculee :  x =  " << velWaistVisp[0] << "    y =  " << velWaistVisp[1] << endl;
